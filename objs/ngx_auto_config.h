#define NGX_CONFIGURE " --with-openssl-opt=no-asm --with-select_module --with-http_sub_module --with-http_v2_module --with-http_flv_module --with-http_mp4_module --with-http_stub_status_module --with-http_gzip_static_module --with-http_realip_module --with-http_dav_module --with-http_gunzip_module --with-http_addition_module --prefix=. --sbin-path=nginx.exe --builddir=objs --with-http_ssl_module --with-mail_ssl_module --with-pcre=objs/pcre2-10.42 --with-openssl=objs/openssl-1.1.1t --with-zlib=objs/zlib-1.2.13 --with-mail --with-http_secure_link_module --with-http_random_index_module --conf-path=conf/nginx.conf --pid-path=logs/nginx.pid --http-log-path=logs/access.log --error-log-path=logs/error.log --http-client-body-temp-path=temp/client_body_temp --http-proxy-temp-path=temp/proxy_temp --http-fastcgi-temp-path=temp/fastcgi_temp --http-scgi-temp-path=temp/scgi_temp --http-uwsgi-temp-path=temp/uwsgi_temp --add-module=objs/nginx-rtmp-module --with-stream"

#ifndef NGX_COMPILER
#define NGX_COMPILER  "gcc 13.1.0 (Rev6, Built by MSYS2 project) "
#endif
